...
class TestOrders(unittest.TestCase):
    ...
    def test_order_from_data_with_transactions(self):
        order = dx.Order(self.order_data, transactions=[
            dx.Transaction(self.security, 100, 100)
        ])
    ...
    def test_order_details_json(self):
        self.assertEqual(self.order_data.__dict__(), {
    ...
    def test_order_json(self):
        self.assertEqual(dx.Order(self.order_data).__dict__(), {
            "data": {
                "security": self.security,
                "price": 100,
                "quantity": 100,
