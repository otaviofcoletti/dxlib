...
class OrderInterface(ExternalInterface):
    ...
    @classmethod
    def map_history(cls, history: History) -> History:  # History[OrderData]
    ...
    def execute_signals(self, signals: Dict[Security, Signal]) -> List[Order]:
        orders_data = self.map_signals(signals)
        return self.execute_orders(orders_data)

    def execute_history(self, history: History) -> History:  # History[Order]
